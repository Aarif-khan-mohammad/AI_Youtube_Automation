...
import {
  ...
  CheckCircle, Circle, Loader2, XCircle, AlertTriangle, Zap,
} from "lucide-react";
...
function StepRow({ step, index, activeStep, status }: {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-lg px-3 py-2.5 transition-all ${
      ...
      <Icon className={`h-4 w-4 shrink-0 ${isActive ? "text-primary" : isDone ? "text-emerald-400" : "text-muted-foreground"}`} />
...
export function PipelineProgress({ initialLogs }: { initialLogs: VideoLog[] }) {
  ...
  const activeJob  = logs.find((l) => ["pending","generating","rendering","uploading"].includes(l.status));
  const displayJob = activeJob ?? logs[0];
  ...
  return (
    <Card className={`border ${isFailed ? "border-red-500/30" : isDone ? "border-emerald-500/30" : activeJob ? "border-primary/30" : "border-border"}`}>
      ...
      <CardContent className="space-y-1">
        {STEPS.map((step, i) => (
          <StepRow key={step.label} step={step} index={i} activeStep={activeStep} status={displayJob.status} />
        ))}
